...
def hillclimb(self):
        ...
        for i in range(self.max_evals):
            candidate = np.array(list(map(mutate, plan)))
...
class Discussion(object):
    ...
    def generate_frequencies(self):
        ...
        """
        self.frequencies = np.array([[random.uniform(0, 50.) for y in range(0, self.num_frequencies)] for x in range(0, self.dimension)], dtype=DTYPE)
        # prself.frequencies
...
def convergence_fixed_paired(runs=100, players=3, landscapes=1,max_it=100,
    ...
    for num_memory in range(min_mem, max_mem+step_size, step_size):
        # 100 runs of each
        for i in range(runs):
